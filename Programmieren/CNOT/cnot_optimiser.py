...
#combine the parametereised modes for a h.
def h_t(h_,t,N_m):
    h = 0
...
def max_h(h_x1,h_x2,h_y1,h_y2,O_max,N_m):
    for i in range(0,N_m):
     upper = np.sqrt(np.power(h_x1,2)+np.power(h_x2,2)+np.power(h_y1,2)+np.power(h_y2,2))/(0.5*O_max)
     max = (1+upper+np.absolute(1-upper))/2
     h_x1=h_x1/max
     h_x2=h_x2/max
     h_y1=h_y1/max
     h_y2=h_y2/max
    return h_x1,h_x2,h_y1,h_y2
